...
function App() {
  ...
  const createJob =async()=>{
    ...
    let res = await fetch("http://localhost:8081/createJob",{method:"POST",body:JSON.stringify(data),headers:{"content-type":"application/json"}});
  ...
  const getData = async()=>{
      let res = await fetch("http://localhost:8081/jobslist",{method:"GET"});
      let json = await res.json();
      console.log(json);
      setJobList(json);
  ...
  const deleteJob = async(id)=>{
    let res = await fetch("http://localhost:8081/deleteJobsById?id="+id,{"method":"delete"})
    if(res.ok){
      alert("Job Deleted");
    }else{
  ...
  const updateJob = async()=>{
    ...
    let res = await fetch("http://localhost:8081/updateJobsById",{method:"POST",body:JSON.stringify(data),headers:{"content-type":"application/json"}});
    let json = await res.json();
